...
class LocalFileSystemProvider(MusicProvider):
        ...
        def run_sync() -> None:
            ...
            try:
                for item in recursive_iter(
                    ...
                    if self._process_item(item, prev_checksum, cur_filenames):
    ...
    def _process_item(
        ...
        :param prev_checksum: Previous checksum from DB, or None if new.
        ...
        try:
            ...
            if item.ext in CUE_EXTENSIONS and self.media_content_type == "music":
                ...
                if item.checksum == prev_checksum:
                    # still need to register synthetic track IDs as current
                    self._register_cue_track_ids(item, cur_filenames)
                    return True

                async def process_cue() -> None:
                ...
                asyncio.run_coroutine_threadsafe(process_cue(), self.mass.loop).result()
                return True
    ...
    def _register_cue_track_ids(
        ...
        # We need to know the track IDs without re-parsing the CUE file.
        # Read and parse the CUE file synchronously (it's a small text file)
        # to get track numbers.
        ...
        try:
            ...
            self.logger.debug("Failed to read CUE file for track ID registration: %s", cue_path)
    ...
    async def _parse_cue_tracks(self, cue_item: FileSystemItem) -> list[Track]:
        ...
        # use a fake FileSystemItem for the audio file to pass to _parse_album
